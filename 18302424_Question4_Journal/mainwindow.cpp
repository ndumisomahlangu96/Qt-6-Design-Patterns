...
// --- Custom Delegate to restrict Author editing inside the Table View ---
class AuthorDelegate : public QStyledItemDelegate {
public:
    AuthorDelegate(QObject *parent = nullptr) : QStyledItemDelegate(parent) {}

    QWidget *createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const override {
        QLineEdit *editor = new QLineEdit(parent);
        // Allows letters, spaces, and hyphens. No full stops or special characters.
...
// --- Custom Delegate to restrict Title/Journal/Volume/Issue editing inside Table View ---
class NonMixedAlphanumericDelegate : public QStyledItemDelegate {
public:
    NonMixedAlphanumericDelegate(QObject *parent = nullptr) : QStyledItemDelegate(parent) {}

    QWidget *createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const override {
        QLineEdit *editor = new QLineEdit(parent);
        // Allows words of purely letters or purely numbers, separated by spaces. Blocks mixed like "3D".
        QRegularExpression rx("^(?:(?:[a-zA-Z]+|[0-9]+)(?:\\s+(?:[a-zA-Z]+|[0-9]+))*\\s*)?$");
        QValidator *validator = new QRegularExpressionValidator(rx, editor);
        editor->setValidator(validator);
        return editor;
    }
...
// --- Custom Delegate to restrict Pages editing inside the Table View ---
class PagesDelegate : public QStyledItemDelegate {
public:
    PagesDelegate(QObject *parent = nullptr) : QStyledItemDelegate(parent) {}

    QWidget *createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const override {
        QLineEdit *editor = new QLineEdit(parent);
        // Allows format "12", "12-15", or "121 - 155". Blocks all alphabetic letters.
        QRegularExpression rx("^\\s*[0-9]+\\s*(?:-\\s*[0-9]+\\s*)?$");
        QValidator *validator = new QRegularExpressionValidator(rx, editor);
        editor->setValidator(validator);
        return editor;
    }
    ...
    , ui(new Ui::MainWindow)
{
    ...
    // --- ADDING VALIDATORS TO UI LINE EDITS ---
    // 1. Author Input Validation
    QRegularExpression authorRegex("^[a-zA-Z\\s\\-]*$");
    QRegularExpressionValidator *authorValidator = new QRegularExpressionValidator(authorRegex, this);
    ui->lineEditAuthor->setValidator(authorValidator);

    // 2. Title, Journal, Volume, and Issue Input Validation
    QRegularExpression mixedAlnumRegex("^(?:(?:[a-zA-Z]+|[0-9]+)(?:\\s+(?:[a-zA-Z]+|[0-9]+))*\\s*)?$");
    QRegularExpressionValidator *nonMixedValidator = new QRegularExpressionValidator(mixedAlnumRegex, this);
    ui->lineEditTitle->setValidator(nonMixedValidator);
    ui->lineEditJournal->setValidator(nonMixedValidator);
    ui->lineEditVolume->setValidator(nonMixedValidator);
    ui->lineEditIssue->setValidator(nonMixedValidator);

    // 3. Pages Input Validation
    QRegularExpression pagesRegex("^\\s*[0-9]+\\s*(?:-\\s*[0-9]+\\s*)?$");
    QRegularExpressionValidator *pagesValidator = new QRegularExpressionValidator(pagesRegex, this);
    ui->lineEditPages->setValidator(pagesValidator);

    // --- APPLYING CUSTOM DELEGATES TO QTABLEVIEW ---
    ui->tableViewDatabase->setItemDelegateForColumn(0, new AuthorDelegate(this));                 // Author
    ui->tableViewDatabase->setItemDelegateForColumn(1, new YearDelegate(this));                   // Year
    ui->tableViewDatabase->setItemDelegateForColumn(2, new NonMixedAlphanumericDelegate(this));   // Title
    ui->tableViewDatabase->setItemDelegateForColumn(3, new NonMixedAlphanumericDelegate(this));   // Journal
    ui->tableViewDatabase->setItemDelegateForColumn(4, new NonMixedAlphanumericDelegate(this));   // Volume
    ui->tableViewDatabase->setItemDelegateForColumn(5, new NonMixedAlphanumericDelegate(this));   // Issue
    ui->tableViewDatabase->setItemDelegateForColumn(6, new PagesDelegate(this));                  // Pages
