    ...
    : QMainWindow(parent)
{
    ...
    // Connect the new XML load button
    connect(btnLoadStudentRecordXMl, &QPushButton::clicked, this, &MainWindow::loadStudentRecordXML);
...
// New logic to load XML and output to textDisplay in the standard format
void MainWindow::loadStudentRecordXML()
{
    // Clear display for a fresh load output
    textDisplay->clear();
    textDisplay->append("🔄 Loading student records from XML...\n");

    // Execute the XML deserialization
    StudentList::getInstance().loadFromXML();

    // Fetch the updated list of students
    const QList<Student*> students = StudentList::getInstance().getList();

    if (students.isEmpty()) {
        textDisplay->append("⚠️ No student records found or XML file is empty.\n");
        return;
    }

    textDisplay->append("✅ Successfully loaded records:\n");

    // Iterate through all students and display them using the standard output format
    for (Student* student : students) {
        QString stdNum = student->getNumber();
